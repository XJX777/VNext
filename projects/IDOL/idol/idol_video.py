...
class IDOL(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.batch_infer_len = 1 #cfg.MODEL.IDOL.BATCH_INFER_LEN
    ...
    def inference(self, outputs, tracker, ori_size, image_sizes):
        ...
        for box in bboxes:
            #box = box.cpu().numpy()
            box = box_cxcywh_to_xyxy(box[:-1])
            box_scaled = [box[0] * ori_size[1], box[1] * ori_size[0] , box[2] * ori_size[1] , box[3] * ori_size[0]]
            bboxes_1.append([box_scaled[0].cpu().numpy(),box_scaled[1].cpu().numpy(),box_scaled[2].cpu().numpy(),box_scaled[3].cpu().numpy()])
    ...
    def preprocess_image(self, batched_inputs):
        ...
        for video in batched_inputs:
            #for frame in video["image"]:
                images.append(self.normalizer(video["image"].to(self.device)))
